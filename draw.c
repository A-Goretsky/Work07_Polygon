...
double calculate(double x0,double y0, double x1,double y1, double x2,double y2) {
	double Ax = x1 - x0;
	double Ay = y1 - y0;
	double bx = x2 - x0;
	double by = y2 - y0;
...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
	...
	clear_screen(s);
	double ** m = polygons->m;
	for (ctr = 0; ctr < polygons->lastcol - 2; ctr+=3) {
		double nz = calculate(m[0][ctr], m[1][ctr], m[0][ctr+1], m[1][ctr+1], m[0][ctr+2], m[1][ctr+2]);
		if (nz > 0) {
			draw_line(m[0][ctr], m[1][ctr], m[0][ctr+1], m[1][ctr+1], s, c);
			draw_line(m[0][ctr+1], m[1][ctr+1], m[0][ctr+2], m[1][ctr+2], s, c);
			draw_line(m[0][ctr+2], m[1][ctr+2], m[0][ctr], m[1][ctr], s, c);
		}
	}
}
	...
	for ( lat = latStart; lat < latStop; lat++ ) {
    	for ( longt = longStart; longt < longStop; longt++ ) {
			...
			add_polygon(
				...
				points->m[1][place],
				points->m[2][place],
				points->m[0][(place + 1) % (points->lastcol)],
				points->m[1][(place + 1) % (points->lastcol)],
				points->m[2][(place + 1) % (points->lastcol)],
				points->m[0][(place + num_steps) % (points->lastcol)],
				points->m[1][(place + num_steps) % (points->lastcol)],
				points->m[2][(place + num_steps) % (points->lastcol)]
			);
			...
			add_polygon(
				...
				points->m[1][(place + 1) % (points->lastcol)],
				points->m[2][(place + 1) % (points->lastcol)],
				points->m[0][(place + num_steps + 1) % (points->lastcol)],
				points->m[1][(place + num_steps + 1) % (points->lastcol)],
				points->m[2][(place + num_steps + 1) % (points->lastcol)],
				points->m[0][(place + num_steps) % (points->lastcol)],
				points->m[1][(place + num_steps) % (points->lastcol)],
				points->m[2][(place + num_steps) % (points->lastcol)]
			);
